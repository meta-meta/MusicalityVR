/*
	...
	Copyright 2016-2025 Sixth Sensor.
	All rights reserved.
	http://sixthsensor.dk
*/
...
namespace OscSimpl
{
	...
	public class OscIn : OscMonoBase
	{
		...
		HashSet<uint> _uniqueAddressesForCurrentFrame = new HashSet<uint>(); // For filtering duplicates. HashSet is faster than List for more than 6 items https://stackoverflow.com/questions/150750/hashset-vs-list-performance
		Dictionary<uint, OscMapping> _regularMappings = new Dictionary<uint, OscMapping>();
		...
		/// </summary>
		public OscReceiveMode mode => _mode;
		...
		/// </summary>
		public string multicastAddress => _multicastAddress;
		...
		/// </summary>
		public static string localIpAddress => OscHelper.GetLocalIpAddress();
		...
		/// </summary>
		public static ReadOnlyCollection<string> localIpAddressAlternatives => OscHelper.GetLocalIpAddressAlternatives();

		/// <summary>
		/// Indicates whether the Open method has been called and the object is ready to receive.
		/// </summary>
		public bool isOpen => _receiver != null && _receiver.isOpen;
		...
		/// </summary>
		public int byteCountReceivedLastFrame => _byteCountLastFrame;
		...
		void Update()
		{
			...
			lock( _lock )
			{
				if( _lockedReceiveBuffer != null && _lockedReceiveBufferByteCount > 0 ) {
			...
			while( index < _safeReceiveBufferByteCount ) {
				OscPacket packet;
				if( OscPacket.TryReadFrom( _safeReceiveBuffer, _safeReceiveBufferByteCount, ref index, out packet ) ) _packetQueue.Enqueue( packet );
			}
			//if( index < _safeReceiveBufferByteCount ){
				//int remainingBytes = _safeReceiveBufferByteCount - index;
				//Debug.Log( "Remaining bytes: " + remainingBytes + ". " + Encoding.ASCII.GetString( _safeReceiveBuffer, index, remainingBytes ) );
				//Debug.Log( "Read bytes: " + index + " / " + _safeReceiveBufferByteCount );
			//}
			...
			if( _filterDuplicates ) _uniqueAddressesForCurrentFrame.Clear();
		}
		...
		void UnpackRecursivelyAndDispatch( OscPacket packet )
		{
			if( packet is OscBundle ) {
				...
				if( _filterDuplicates ) {
					...
					if( _uniqueAddressesForCurrentFrame.Contains( addressHash ) ) {
						OscPool.Recycle( message );
						return;
					}
					_uniqueAddressesForCurrentFrame.Add( addressHash );
				}
		...
		void Dispatch( OscMessage message )
		{
			...
			// Regular mappings.
			OscMapping mapping;
			if( _regularMappings.TryGetValue( message.GetAddressHash(), out mapping ) ) { // Getting an interger hash and using it as lookup key instead of a string is faster.
		...
		void OnDataReceivedAsync( byte[] data, int byteCount )
		{
			...
			lock( _lock )
			{
				...
				// If this is not the first buffer to be received within this update, then pad with four zeros.
				// We need zero padding between messages because if the last argument is a string, then we could mistake the next message as part of that string.
		...
		void UpdateMappings()
		{
			...
			foreach( OscMapping mapping in _mappings )
			{
				...
				if( mapping.hasSpecialPattern ){
					...
					uint addressHash = mapping.GetAddressHash();
					if( _regularMappings.ContainsKey( addressHash ) ) continue;
					_regularMappings.Add( addressHash, mapping );
				}
			}
